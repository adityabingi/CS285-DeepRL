...
class ReplayBuffer(object):
    ...
    def add_rollouts(self, paths, noised=False):
        ...
        observations, actions, next_observations, terminals, concatenated_rews, unconcatenated_rews, logprobs = convert_listofrollouts(paths)
        ...
        if self.obs is None:
            ...
            self.logprobs = logprobs[-self.max_size:]
        else:
            ...
            self.logprobs = np.concatenate([self.logprobs, logprobs])[-self.max_size:]
    ...
    def sample_random_data(self, batch_size):
        ...
        assert self.obs.shape[0] == self.acs.shape[0] == self.concatenated_rews.shape[0] == self.next_obs.shape[0] == self.terminals.shape[0] == self.logprobs.shape[0]
        rand_indices = np.random.permutation(self.obs.shape[0])[:batch_size]
        return self.obs[rand_indices], self.acs[rand_indices], self.concatenated_rews[rand_indices], self.next_obs[rand_indices], self.terminals[rand_indices], self.logprobs[rand_indices]

    def sample_recent_data(self, batch_size=1, concat_rew=True):

        if concat_rew:
            return self.obs[-batch_size:], self.acs[-batch_size:], self.concatenated_rews[-batch_size:], self.next_obs[-batch_size:], self.terminals[-batch_size:], self.logprobs[-batch_size:]
        else:
            ...
            observations, actions, next_observations, terminals, concatenated_rews, unconcatenated_rews, logprobs = convert_listofrollouts(rollouts_to_return)
            return observations, actions, unconcatenated_rews, next_observations, terminals, logprobs